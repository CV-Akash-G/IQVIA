...
try:
    subprocess.run(["python", "IQ_Agent.py"], check=True)
...
streamlit_process = subprocess.Popen(["streamlit", "run", "Rules_Feed_Agnent.py"])
...
try:
    subprocess.run(["python", "Analyze_Agent.py"], check=True)
    logging.info("Reports generation script completed successfully.")
except subprocess.CalledProcessError as e:
    logging.error(f"Error running reports generation script: {e}")
    raise

# Step 7: Run the final script (text_correction.py)
logging.info("Running text correction script (text_correction.py)...")
try:
    subprocess.run(["python", "Correction_Agent.py"], check=True)
...
try:
    subprocess.run(["python", "Curated_Analyze_Agent.py"], check=True)
    logging.info("Reports generation script completed successfully.")
except subprocess.CalledProcessError as e:
    logging.error(f"Error running reports generation script: {e}")
    raise
